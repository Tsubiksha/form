...
import { CheckCircle2, Zap, Shield, Mail, Lock, User, Eye, EyeOff, Sparkles, ArrowRight, ShieldCheck, BarChart3 } from "lucide-react";
...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    else if (!validPassword(form.password)) next.password = "Password must be at least 6 characters (letters and numbers only)";
    setErrors(next);
    ...
    try {
      ...
      setErrors({ form: apiMessage(err, "Registration failed. Please try again.") });
    } finally {
      setBusy(false);
    }
  };
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 500, margin: '40px 0' }}>
          <div style={{
            ...
            <Sparkles size={14} />{t('ui.start_free_workspace', `Start Free Workspace`)}</div>

          <h1 style={{ fontSize: 36, fontWeight: 800, color: 'white', letterSpacing: '-0.03em', lineHeight: 1.25, marginBottom: 16 }}>{t('ui.join_thousands_of_teams_building_modern', `Join thousands of teams building modern data collection workflows.`)}</h1>

          <p style={{ fontSize: 15, color: '#94a3b8', lineHeight: 1.6, marginBottom: 36 }}>{t('ui.create_your_account_today_and_gain_immed', `Create your account today and gain immediate access to our drag-and-drop form builder, automated retention rules, and real-time response analytics.`)}</p>
        ...
        <div style={{ position: 'relative', zIndex: 2, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, color: '#64748b', fontWeight: 500 }}>{t('ui.no_credit_card_required_free_14_day_tria', `No credit card required · Free 14-day trial`)}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '56px 40px', background: 'var(--bg-surface)' }}>
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em', margin: '0 0 8px 0' }}>{t('ui.create_your_account', `Create Your Account`)}</h2>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: 0 }}>{t('ui.get_started_for_free_set_up_your_workspa', `Get started for free. Set up your workspace in seconds.`)}</p>
          </div>
          ...
          <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text-primary)' }}>{t('ui.full_name', `Full Name`)}</label>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  autoComplete="name"
                  autoFocus
                  placeholder={t('ui.jane_smith', `Jane Smith`)}
                  style={{
                    width: '100%',
                    height: 44,
                    border: errors.name ? '1px solid var(--danger-500)' : '1px solid var(--border-subtle)',
                    borderRadius: 8,
              ...
              {errors.name && <span style={{ fontSize: 12, color: 'var(--danger-600)', fontWeight: 500 }}>{errors.name}</span>}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  placeholder={t('ui.min_6_characters', `Min 6 characters`)}
                  style={{
                    width: '100%',
                    height: 44,
            ...
            >
              {busy ? "Creating account…" : <>{t('ui.create_account', `Create Account`)}<ArrowRight size={16} /></>}
            </button>
          </form>

          <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-secondary)', marginTop: 28 }}>{t('ui.already_have_an_account', `Already have an account?`)}{" "}
            <Link to="/login" style={{ color: 'var(--brand-600)', fontWeight: 600, textDecoration: 'none' }}>{t('ui.sign_in', `Sign In`)}</Link>
          </p>
        </div>
      </div>
    </div>
